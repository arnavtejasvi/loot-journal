...
public final class LootJournalSession {
    ...
    private static void showBreakFeedback(Minecraft mc, BlockBreakEvent event) {
        ...
        mc.gui.getChat().addClientSystemMessage(Component.literal("§e[Randomizer] §f" + blockName + " §8→ " + drops));
    }
    ...
    private static void printSummary(Minecraft mc, SessionReport session) {
        ...
        mc.gui.getChat().addClientSystemMessage(Component.literal("§6=== Loot Journal Session Summary ==="));
        mc.gui.getChat().addClientSystemMessage(Component.literal("§7Session: §f" + session.worldName + " §8(" + elapsed + ")"));
        mc.gui.getChat().addClientSystemMessage(Component.literal("§aGained: " + formatItemMap(session.totalGained)));
        mc.gui.getChat().addClientSystemMessage(Component.literal("§cLost: "   + formatItemMap(session.totalLost)));
        mc.gui.getChat().addClientSystemMessage(Component.literal("§eXP gained: +" + session.xpGained));
        if (!session.mobKills.isEmpty()) {
            mc.gui.getChat().addClientSystemMessage(Component.literal("§bMobs killed: " + formatMobKills(session.mobKills)));
        }
        mc.gui.getChat().addClientSystemMessage(Component.literal("§7Blocks broken: " + session.blockAggregates.size() + " type(s)"));
        if (LootJournalConfig.writeSessionJson) {
            mc.gui.getChat().addClientSystemMessage(Component.literal("§7Session saved to loot-journal/sessions/"));
        }
    }
