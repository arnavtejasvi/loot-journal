...
public class SessionHistoryScreen extends Screen {
    ...
    public void extractRenderState(GuiGraphicsExtractor g, int mouseX, int mouseY, float delta) {
        ...
        g.text(this.font, this.title, this.width / 2 - titleW / 2, 8, 0xFFFFFF);

        int detailTop = this.height - DETAIL_HEIGHT - 28;
        g.fill(0, detailTop, this.width, detailTop + DETAIL_HEIGHT, 0x88000000);
        g.fill(0, detailTop, this.width, detailTop + 1, 0xFF555555);

        if (selected != null) {
            renderDetail(g, selected, detailTop + 4);
        } else if (sessionList.getEntryCount() == 0) {
            String noHistoryMsg = "No session history found. Sessions are saved when writeSessionJson is enabled.";
            int noHistoryW = this.font.width(noHistoryMsg);
            g.text(this.font, noHistoryMsg, this.width / 2 - noHistoryW / 2, HEADER_HEIGHT + 20, 0xAAAAAA);
            String clickMsg = "Click a session to see details";
            int clickW = this.font.width(clickMsg);
            g.text(this.font, clickMsg, this.width / 2 - clickW / 2, detailTop + DETAIL_HEIGHT / 2 - 4, 0x888888);
        } else {
            String clickMsg = "Click a session to see details";
            int clickW = this.font.width(clickMsg);
            g.text(this.font, clickMsg, this.width / 2 - clickW / 2, detailTop + DETAIL_HEIGHT / 2 - 4, 0x888888);
    ...
    private void renderDetail(GuiGraphicsExtractor g, SessionReport s, int y) {
        int x = 8;
        ...
        g.text(this.font, header, x, y, 0xFFFFAA);
        y += rowH + 1;

        g.text(this.font, "§aGained: §f" + LootJournalSession.formatItemMap(s.totalGained != null ? s.totalGained : Map.of()), x, y, 0xFFFFFF);
        g.text(this.font, "§eXP: §f+" + s.xpGained, col2, y, 0xFFFFFF);
        y += rowH;

        if (s.mobKills != null && !s.mobKills.isEmpty()) {
            int total = s.mobKills.values().stream().mapToInt(Integer::intValue).sum();
            g.text(this.font, "§bMobs: §f" + formatKillsSummary(s.mobKills, total), x, y, 0xFFFFFF);
        } else {
            g.text(this.font, "§7No mob kills recorded", x, y, 0xFFFFFF);
        }
        int blocks = s.blockAggregates != null ? s.blockAggregates.size() : 0;
        g.text(this.font, "§7Blocks: §f" + blocks + " type(s)", col2, y, 0xFFFFFF);
    }
    ...
    static class SessionList extends ObjectSelectionList<SessionList.SessionEntry> {
        ...
        static class SessionEntry extends ObjectSelectionList.Entry<SessionEntry> {
            ...
            @Override
            public void extractContent(GuiGraphicsExtractor g, int x, int y, boolean hovered, float delta) {
                ...
                g.text(mc.font, "§e" + date + "  §f" + world, x + 2, y + 2, 0xFFFFFF);
                g.text(mc.font, "§7" + dur + "  §a+" + gained + " items  §b" + mobs + " mobs",
                        x + 2, y + 12, 0xFFFFFF);
            }
